...
class Controller_Login extends Controller_Base {
		...
		if (isset($_POST) && Valid::not_empty($_POST)) {
			...
			if ($post->check() && Auth::instance()->login($post['username'], $post['password'], $remember))
			{
				if(Auth::instance()->logged_in()) {
					// sucessfully loged
					$this->request->redirect('/painel/projetos');
				}
			} else {
				// wrong username or password (but form is valid)
				$loginerrors =  __('Login ou senha inválidos');
			}
			// validation failed, collect the errors
			$errors = $post->errors('user');
		}
		$base_params = array('inner_view' => View::factory('account/index', array('post'=> $post,'errors'=> $errors,'loginerrors'=> $loginerrors)));
    	$this->response->body(View::factory('labsec/base_painel', $base_params)->render());
    }
	...
	public function action_create()
	{
		...
		$base_params = array('inner_view' => View::factory('account/create', array('post'=>$post,'errors'=>$errors)));
    	$this->response->body(View::factory('labsec/base_painel', $base_params)->render());
...
//		// display
//    	$this->response->body(View::factory('account/login', array(
//	    	'post'=> $post,
//	    	'errors'=> $errors,
//	    	'loginerrors'=> $loginerrors
//    	))->render());
    	
		$base_params = array('inner_view' => View::factory('account/login', array('post'=> $post,'errors'=> $errors,'loginerrors'=> $loginerrors)));
    	$this->response->body(View::factory('labsec/base_painel', $base_params)->render());
...
//			->bind('loginerrors', $loginerrors);
	...
	public function action_reset()
	{
		...
		$base_params = array('inner_view' => View::factory('account/reset', array('post'=>$post,'errors'=>$errors,'sent'=>$sent,'title'=>'Reset password')));
    	$this->response->body(View::factory('labsec/base_painel', $base_params)->render());
